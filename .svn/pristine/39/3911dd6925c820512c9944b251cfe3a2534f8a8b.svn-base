...
class AlexaUtilService {
		...
		public String sha1Digest (byte[] data) throws SignatureException {
			return getDigest("SHA-1", data, false);
		}
		   
		private String getDigest(String algorithm, byte[] data, boolean toLower)
	    ...
	    def checkRequestSignature(final byte[] serializedSpeechletRequest,
	        ...
	        try {
				...
				rsa.init(Cipher.DECRYPT_MODE, signingCertificate.getPublicKey());
				byte[] utf8 = rsa.doFinal(decodedBase64Sig);
				def decryptedDecodedSignature=  new String(utf8, "UTF8");
				log.warn("decryptedDecodedSignature:" + decryptedDecodedSignature)
				
	            //signature.update(serializedSpeechletRequest);
	           	
				def sha1HashValueOfRequestBody = sha1Digest(serializedSpeechletRequest)
				log.warn("sha1HashValueOfRequestBody:" + sha1HashValueOfRequestBody)
